...
describe('Customers Cart', () => {
    ...
    test('get all the customers cart information to display', async () => { // eslint-disable-line require-await
        ...
        const customerResponse = await frisby.get('http://localhost:3000/customers/${customers.customerId}');
        const actualCustomer = customerResponse.json;

        constCustomersCartResponse = await frisby.get('http://localhost:3000/customers/${customer.customerId}/cats');
        ...
        const firstItemResponse = await friby.get('http://localhost:3000/items/${actualFirstCartItem.itemId}');
